...
- Builder v0.5 fixture implementation is isolated in `causebase_builder.v05`: typed models, `adapt_rc4_fixture`, and independent validation cover the four frozen fixtures. Its tests and the legacy RC4 suite pass; it does not change RC4 production publication.
- Full RC4→0.5 promotion is currently blocked at Gate A: the immutable 120-card RC4 release has 402 activity, 226 beneficiary and 218 descriptive-geography observation records with no evidence IDs, plus 573 provenance-free classifications. The approved migration rules prohibit both inventing evidence and silently dropping them. See `MIGRATION_V05_AUDIT.md`; do not cut Viewer over until an explicit product decision resolves this.
