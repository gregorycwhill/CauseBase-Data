...
- Golden Corpus v1 is now committed as a 21-case governed manifest (seven hard-gold, fourteen review-required) without PDFs or private extraction outputs. Its first document-stack run conditionally selects Document Pipeline v2: cached `pdfplumber` native text, positioned blocks and table structures; low-text OCR and visual analysis are explicit optional routes, not silent fallback. EJA labels were recovered and 59 table structures detected, but the visual allocation was only 3/4 marker recall and no local OCR engine was installed. See GOLDEN_CORPUS.md and DOCUMENT_EXTRACTION_STACK_DECISION.md.
- The superseding ecosystem bake-off installed and evaluated independent local components: PyMuPDF, Camelot stream/lattice, Tesseract 5.5.3 and RapidOCR ONNX alongside `pdfplumber`. The primary route now passes EJA P&L 33/33 and financial position 32/32 with exact source-faithful checks; both OCR engines recover genuine low-text Golden Corpus pages. PyMuPDF is not adopted because of AGPL/commercial licensing; Docling failed the current OneDrive-path Windows install due to path length. The computed Document Gate remains **CONDITIONAL** solely because EJA’s 4/4 chart association is unvalidated: the narrow configured OpenAI vision test would transmit one private report page and needs explicit authority. Do not start website/identity work until that gate is green. See DOCUMENT_EXTRACTION_ECOSYSTEM_REVIEW.md and DOCUMENT_EXTRACTION_STACK_DECISION.md.
