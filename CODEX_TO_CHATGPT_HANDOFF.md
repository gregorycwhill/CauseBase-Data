...
- Draft public contract `0.5` is ready for product review: `PUBLIC_SCHEMA_VNEXT_SPEC.md` defines the object/release/observation/financial/coverage/privacy contract; `RC4_TO_VNEXT_COMPATIBILITY.md` defines one-release dual-projection migration; `schemas/vnext/` and `examples/vnext/` provide draft machine structures and EJA/sparse/identity/multi-period examples. It is not wired to Builder or Viewer.
