...
- Golden Corpus v1 is established (21 governed cases; seven hard-gold and fourteen review-required). A real six-component local ecosystem bake-off now selects `pdfplumber` for source-faithful digital financial syntax (EJA P&L 33/33; financial position 32/32) and local Tesseract for low-text OCR. Camelot does not displace the baseline; RapidOCR remains an alternative. The document state is **conditional**, not green, because the EJA 4/4 visual chart route awaits explicit authority for a page-scoped external vision test. Website/identity work has not begun. See DOCUMENT_EXTRACTION_ECOSYSTEM_REVIEW.md and DOCUMENT_EXTRACTION_STACK_DECISION.md.
