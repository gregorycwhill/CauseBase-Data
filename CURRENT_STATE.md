...
- RC4 remains immutable history. Its representation was migrated without fresh enrichment; the v0.5 release and Viewer select the current immutable Data release rather than treating RC4 as the live contract.
...
- Golden Corpus v1 is established (21 governed cases; seven hard-gold and fourteen review-required). A real six-component local ecosystem bake-off selects `pdfplumber` for source-faithful digital financial syntax (EJA P&L 33/33; financial position 32/32), local Tesseract for low-text OCR, and a local vector colour/geometry route for EJA’s visual allocation (4/4). The computed Document Gate is **DECISIVE**. A bounded private Evidence Engine pilot then completed all seven retained PDFs, twelve retained website subjects and all 21 identity/fundraising cases without card writes, crawl expansion or public release mutation. See DOCUMENT_EXTRACTION_ECOSYSTEM_REVIEW.md, DOCUMENT_EXTRACTION_STACK_DECISION.md and EVIDENCE_ENGINE_PILOT.md.

- Knowledge Validation v1 deterministic preparation is complete and awaiting the required human gate: 102 review-only website candidates were inventoried and a private 48-case stratified semantic package was prepared. No semantic candidate, taxonomy change, fundraising value, corpus rebuild or Viewer deployment has occurred. See KNOWLEDGE_VALIDATION_V1.md, TAXONOMY_VALIDATION_V1.md and KNOWLEDGE_VALIDATION_HUMAN_GATE.md.
